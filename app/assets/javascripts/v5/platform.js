...
$(function(){
    ...
    // img scroll
    (function(){
        var div = $(".p3-show-wrapp");
        var ul = div.find("ul");
        var li = div.find("li");
        var len = li.length;
        var widthItems = 257 + 15 * 2;
        var widthCenter = 320 + 120 *2;
        var widthList = len%2 ? ((len - 1) * widthItems + widthCenter) : (len * widthItems + widthCenter); // 个数不为奇数则补充一个单元的长度
        var centerIndex = Math.floor(len/2);

        var curr = 0;
        var minCurr = -centerIndex;
        var maxCurr = len%2 ? centerIndex : (centerIndex-1);
        var bigIndex = centerIndex;

        var itemCurr = curr + centerIndex;
        var txtLi = $(".p3-txt");

        function scroll(){
            itemCurr = curr + centerIndex;
            txtLi.eq(itemCurr).stop(true, false).fadeIn().siblings(".p3-txt").stop(true, false).fadeOut();

            ul.stop(true, false).animate({
                left: -widthItems * curr
            });

            li.filter(".curr").stop(true, false).animate({
                width: 257,
                height: 455,
                "margin-left": 15,
                "margin-right": 15,
            ...
            li.eq(bigIndex).stop(true, false).animate({
                width: 320,
                height: 568,
                "margin-left": 120,
                "margin-right": 120,
            ...
            li.removeClass("curr").eq(bigIndex).addClass("curr");
        }
        ...
        scroll();
        $(".p3-show").on("click", ".p3-trip-r", function(){
            curr++;
            if(curr > maxCurr)
            {
                curr = maxCurr;
