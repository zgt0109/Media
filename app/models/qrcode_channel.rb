class QrcodeChannel < ActiveRecord::Base
  ...
  validates :qrcode_channel_type_id, presence: true
  ...
  def self.chart_data(total,date,today,select_time,params,total_qrcode_logs,qrcode_channels)
    count = 0
    series = []
    categories = []
    if select_time
      ...
      # total.each do |qrcode_channel|
      qrcode_channels.each do |qrcode_channel|
        hash = {'name' => qrcode_channel.name}
        # categories,hash['data'],count = QrcodeChannel.get_select_date(total_qrcode_logs,qrcode_channel,start_time,end_time,count)
        categories,hash['data'] = QrcodeChannel.get_select_date(total_qrcode_logs,qrcode_channel,start_time,end_time)
        series << hash
      end
    else
      # total.each do |qrcode_channel|
      qrcode_channels.each do |qrcode_channel|
        hash = {'name' => qrcode_channel.name}
        # categories,hash['data'],count,start_time,end_time = QrcodeChannel.get_date(total_qrcode_logs,qrcode_channel,date,today,count)
        categories,hash['data'],start_time,end_time = QrcodeChannel.get_date(total_qrcode_logs,qrcode_channel,date,today)
        series << hash
      end
    end
    count = total_qrcode_logs.qrcode_channel_normal(total.pluck(:id)).select_time(start_time, end_time).select(:id).count
    min_tick = categories.length > 7 ? 6 : nil
    [categories, series, start_time, end_time, count, min_tick]
  end

  # def self.get_date_count(total_qrcode_logs,qrcode_channel,date,today,count)
  #   total = total_qrcode_logs.qrcode_channel_normal(qrcode_channel.id).send(date, today)
  #   count += total.length
  #   count
  # end

  # def self.get_select_date_count(total_qrcode_logs,qrcode_channel,start_time,end_time,count)
  #   total = total_qrcode_logs.qrcode_channel_normal(qrcode_channel.id).select_time(start_time, end_time)
  #   count += total.length
  #   count
  # end

  def self.get_select_date(total_qrcode_logs,qrcode_channel,start_time,end_time)
    ...
    if diff_time <= 31
      ...
      categories,data = QrcodeChannel.get_hash_day(h,qrcode_logs)
      ...
      categories,data = QrcodeChannel.get_hash_month(h,qrcode_logs)
    elsif diff_time > 365
      ...
      categories,data = QrcodeChannel.get_hash_year(h,qrcode_logs)
    end
    # return categories,data,count
    return categories,data
  end

  def self.get_date(total_qrcode_logs,qrcode_channel,date,today)
    h = {}
    total = total_qrcode_logs.qrcode_channel_normal(qrcode_channel.id).send(date, today)
    # count += total.length
    if date == "one_weeks"
  ...
  def self.chart_data_amount(total,date,today,select_time,params,qrcode_channels)
    amount = 0
    series = []
    categories = []
    if select_time
      ...
      qrcode_channels.each do |qrcode_channel|
        hash = {'name' => qrcode_channel.name}
        # categories,hash['data'],amount = QrcodeChannel.get_select_date_amount(qrcode_channel,start_time,end_time,amount)
        categories,hash['data'] = QrcodeChannel.get_select_date_amount(qrcode_channel,start_time,end_time)
        series << hash
      end
      ...
      qrcode_channels.each do |qrcode_channel|
        hash = {'name' => qrcode_channel.name}
        categories,hash['data'],start_time,end_time = QrcodeChannel.get_date_amount(qrcode_channel,date,today)
        series << hash
      end
      # total.each do |qrcode_channel|
      #   total_qrcode_users = qrcode_channel.qrcode.qrcode_users.send(date, today) rescue QrcodeUser.none
  ...
  # def self.get_select_date_amount(qrcode_channel,start_time,end_time,amount)
  def self.get_select_date_amount(qrcode_channel,start_time,end_time)
    diff_time = (start_time - end_time).to_i
    h = {}
    total = qrcode_channel.qrcode.qrcode_users.select_time(start_time, end_time)
    ...
    if diff_time <= 31
      ...
      categories,data = QrcodeChannel.get_hash_day_amount(h,qrcode_users)
    elsif diff_time > 31 &&  diff_time < 365
      ...
      categories,data = QrcodeChannel.get_hash_month_amount(h,qrcode_users)
    elsif diff_time > 365
      ...
      categories,data = QrcodeChannel.get_hash_year_amount(h,qrcode_users)
    end
    # return categories,data,amount
    return categories,data
  end

  # def self.get_date_amount(qrcode_channel,date,today,amount)
  def self.get_date_amount(qrcode_channel,date,today)
    h = {}
    total = qrcode_channel.qrcode.qrcode_users.send(date, today) rescue QrcodeUser.none
    # amount += total.sum(:total_amount)
    if date == "one_weeks"
      qrcode_users = total.select('created_date, sum(total_amount) as amount').group('created_date').order("created_at asc")
      start_time = today - 6.day
