class WxShakePrize < ActiveRecord::Base
  belongs_to :supplier
  belongs_to :wx_user
  belongs_to :wx_shake
  belongs_to :wx_shake_round
  belongs_to :wx_shake_user
  ...
  def self.export_excel(search,wx_shake_round_id)
    xls_report = StringIO.new
    book = WxShakePrize.new_excel
    book_excel = book[0]
    book_sheet = book[1]
    ...
    search.each_with_index do |s, index|
      prize = s.wx_shake_prizes.where(wx_shake_round_id: wx_shake_round_id).first
