class ShopOrder < ActiveRecord::Base
  ...
  def send_message
    ...
    if book_dinner?
      ...
      sms_options = { mobiles: shop_branch.mobile, template_code: 'SMS_7471070', params: { username: user.name, mobile: shop_branch.mobile, total_amount: total_amount, address: address, shop_branch_name: shop_branch.name, order_items: shop_order_items.collect{|item| "#{item.product_name}×#{item.qty} ￥#{item.total_price}" }.join('，') } }
    end
