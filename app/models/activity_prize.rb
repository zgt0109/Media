    ...
    def create_red_packet
      ...
      if activity_red_packet
        ...
        attr = {
            ...
            supplier_id: activity.try(:supplier).try(:id),
            payment_type_id: PaymentType::WX_REDPACKET_PAY,
            act_name: "#{activity.name}#{title}",
            nick_name: activity.try(:supplier).try(:wx_mp_user).try(:name),
            send_name: activity.try(:supplier).try(:wx_mp_user).try(:name),
            wishing: '恭喜发财,大吉大利',
            remark: '微枚迪技术支持',
            min_value: prize_value,
            max_value: prize_value,
