class WeixinCard
  class << self
    def card_pass_check(wx_mp_user, xml)
      wx_card = wx_mp_user.cards.where(card_id: xml[:CardId]).first
    ...
    def user_get_card(wx_user, wx_mp_user, xml)
      ...
      wx_mp_user.site.consumes.hidden.where('consumes.id <> ?', consume.id).wx_card_consumes(wx_user.id).update_all(status: Consume::DELETED)
      ''
    end

    def user_del_card(wx_mp_user, xml)
      consume = wx_mp_user.site.consumes.where(code: xml[:UserCardCode]).first
