class Pro::CarOwnersController < ApplicationController
	...
	def index
		now = Time.now
		@car_activity_notice = current_site.car_activity_notices.where(notice_type: 7).first || current_site.car_activity_notices.new(site_id: current_site.id, wx_mp_user_id: current_site.wx_mp_user.id, car_shop_id: @car_shop.id, notice_type: 7)
