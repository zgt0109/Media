...
class Pro::DoctorWatchesController < Pro::HospitalBaseController
  ...
  def index
    if current_site.shop.blank? || current_site.shop.shop_branches.used.count == 0
  ...
  def edit
    @doctor_watch = current_site.hospital.doctor_watches.find(params[:id])
  ...
  def update
    ...
    if temp_doctor_watch.is_multi
      ...
      current_site.hospital.doctor_watches.find(params[:id]).update_attributes(params[:doctor_watch])
    end
