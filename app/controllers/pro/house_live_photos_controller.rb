class Pro::HouseLivePhotosController < Pro::HousesBaseController
  def index
    case params[:filter]
    when 'approved'
      @live_photos = current_site.house.live_photos.where(status: 'approved').order("created_at desc").page(params[:page])
    when 'unapproved'
      @live_photos = current_site.house.live_photos.where("status != 'approved' or status is null").order("created_at desc").page(params[:page])
    else
      @live_photos = current_site.house.live_photos.order("created_at desc").page(params[:page])
    end
    @activity = current_site.create_activity_for_house_live_photo
  end
  ...
  def approve
    @live_photo = current_site.house.live_photos.find(params[:id])
