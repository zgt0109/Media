class Pro::HouseImpressionsController < Pro::HousesBaseController
  ...
  def index
    @impressions = current_site.house.impressions.where(predefined: true).order('ratio desc').page(params[:page])
    @activity = current_site.create_activity_for_house_impression
  end
  ...
  def create
    @impression = current_site.house.impressions.build(params[:house_impression].merge(predefined: true))
    if @impression.save
    flash[:notice] = "保存成功"
  ...
  def edit
    @impression = current_site.house.impressions.find(params[:id])
    render layout: 'application_pop'
  end
