class Pro::HouseIntrosController < Pro::HousesBaseController
  def index
    @intro = current_user.house.intro || current_user.house.build_intro
    @activity = current_user.wx_mp_user.create_activity_for_house_intro
  end
  ...
  def create
    @intro = current_user.house.build_intro params[:house_intro]
    if @intro.save
  ...
  def update
    @intro = current_user.house.intro
