class Pro::TripsController < Pro::TripBaseController
  ...
  def ads
    @trip_ads = current_user.trip.trip_ads.order(:sort) || []
  end

  def ad_add
    sorts = current_user.trip.trip_ads.pluck(:sort)
  ...
  def up_ad_title
    trip_ad_params = HashWithIndifferentAccess.new(params[:trip_ad] || {})
    ad = current_user.trip.trip_ads.find(trip_ad_params[:id])
    ad.title = trip_ad_params[:title]
    ad.save
  ...
  def del_ad
    ad = current_user.trip.trip_ads.find(params[:id])
    ad.destroy
    redirect_to trips_path(anchor: "tab-2")
  end

  def ticket
    @category = current_user.trip_ticket_categories.where(id: params[:trip_ticket_category_id]).first
    @current_titles << '门票管理'
    @search = current_user.trip.trip_tickets.visible.categorized(@category).latest.search(params[:search])
    @trip_tickets = @search.page(params[:page])
  end

  def new_ticket
    @trip_ticket = current_user.trip.trip_tickets.new
  end

  def save_ticket
    @current_titles << '门票管理'
    ticket_id = params[:ticket_id].presence
    @trip_ticket = ticket_id ? current_user.trip.trip_tickets.find(ticket_id) : current_user.trip.trip_tickets.new
  ...
  def destroy_ticket
    @trip_ticket = current_user.trip.trip_tickets.find(params[:ticket_id])
    @trip_ticket.deleted!
    redirect_to ticket_trips_path, notice: '操作成功'
  end

  def show_ticket
    @trip_ticket = current_user.trip.trip_tickets.find(params[:id])
  end
  ...
  def order
    ...
    @search = current_user.trip.trip_orders.order('created_at desc').search(search_params)
  ...
  def show_order
    @trip_order = current_user.trip.trip_orders.find(params[:id])
