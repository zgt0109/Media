class Pro::WxPlotCategoriesController < Pro::WxPlotBaseController
  ...
  def sms_setting
    ...
    @system_message_setting = current_user.system_message_settings.includes(:system_message_module).where(["system_message_modules.module_id = ?", params[:module_id]]).first_or_initialize(supplier_id: current_user.id, system_message_module_id: SystemMessageModule.where(module_id: params[:module_id]).first.id)
  end
