class Mobile::EcOrdersController < Mobile::BaseController
  ...
  def index
    @orders = @wx_user.orders.where(site_id: session[:site_id]).order('id DESC')
  end
  ...
  def new
    ...
    @carts = params[:items].map{|item| EcCart.create(site_id: @site.id, user_id: session[:user_id], ec_item_id: item, qty: 1)} unless @carts.present?
