class Mobile::WavesController < Mobile::BaseController
  ...
  def check
    user_lottery_draws = @activity.lottery_draws.where(user_id: session[:user_id])
    if (@activity.activity_property.day_partake_limit == -1) || (user_lottery_draws.today.count < @activity.activity_property.day_partake_limit) #每人每天参与次数
      if (@activity.activity_property.partake_limit == -1) || (user_lottery_draws.count < @activity.activity_property.partake_limit)#每人参与总次数
        if @activity.activity_property.prize_limit == -1 or user_lottery_draws.win.count < @activity.activity_property.prize_limit #每人总中奖次数
          if @activity.activity_property.day_prize_limit == -1 or user_lottery_draws.today.win.count < @activity.activity_property.day_prize_limit #每人每天中奖次数
          else
  ...
  def get_prize
    ...
    if @activity.setted? and @activity.activity_status == Activity::UNDER_WAY
      ...
      user_lottery_draws = lottery_draws.where(user_id: user_id)
      if @activity.activity_property.day_partake_limit == -1 or user_lottery_draws.today.count < @activity.activity_property.day_partake_limit #每人每天参与次数
        if @activity.activity_property.partake_limit == -1 or user_lottery_draws.count < @activity.activity_property.partake_limit #每人参与总次数
          if @activity.activity_property.day_prize_limit == -1 or user_lottery_draws.today.win.count < @activity.activity_property.day_prize_limit #每人每天中奖次数
            if @activity.activity_property.prize_limit == -1 or user_lottery_draws.win.count < @activity.activity_property.prize_limit #每人总中奖次数
              ...
              if prize #如果中奖.where(user_id: user_id
                logger.info "========获取到奖券==========="
                if prize.people_day_limit_count == -1 or user_lottery_draws.where(activity_prize_id: prize.id).today.count < prize.people_day_limit_count # 某奖品每人每天次数
                  if prize.people_limit_count == -1 or user_lottery_draws.where(activity_prize_id: prize.id).count < prize.people_limit_count # 某奖品每人总次数
  ...
  def prizes
    @activity_consumes = @user.present? ? @user.activity_consumes.includes(:activity_prize).where(activity_id: @activity.id).order(:created_at) : []
  end
