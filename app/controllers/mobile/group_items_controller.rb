class Mobile::GroupItemsController < Mobile::BaseController
  ...
  def order
    ...
    unless @group_item.limit_coupon_count == -1
      redirect_to mobile_group_item_path(supplier_id: @supplier.id, id: @group_item), :notice => "此商品每人每天最多只能购买#{@group_item.limit_coupon_count}件" if @group_orders.sum(&:qty) >= @group_item.limit_coupon_count
    end
  ...
  def set_group_item
    ...
    redirect_to mobile_groups_path(supplier_id: @supplier), :notice => "此商品不存在或已下架" unless @group_item.present?
  end

  def redirect_vcoolife
    return redirect_to "#{WMALL_HOST}/wx/gotogroupdetail/#{@group_item.groupable_id}-#{@group_item.id}?supplier_id=#{@supplier.id}"if @group_item.present? && @group_item.groupable_type == "Wmall::Shop"
  end
