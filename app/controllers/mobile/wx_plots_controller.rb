class Mobile::WxPlotsController < Mobile::BaseController
  ...
  def bulletin
    ...
    @share_image = @bulletin.pic_key.present? ? @bulletin.pic_url : @wx_plot.activity_wx_plot_bulletin.try(:qiniu_pic_url)
    render layout: 'mobile/wx_plot_website'
  ...
  def repair_complains
    @share_image = params[:type] == 'repair' ? @wx_plot.activity_wx_plot_repair.try(:qiniu_pic_url) : @wx_plot.activity_wx_plot_complain.try(:qiniu_pic_url)
  end
  ...
  def create_repair_complain
    ...
    if @repair_complain.save
      @site.account.send_system_message({site_id: @site.id, content: "#{@repair_complain.created_at.strftime('%H:%M')}收到#{@repair_complain.nickname}用户#{@repair_complain.phone}的#{@repair_complain.wx_plot_category.name}#{@repair_complain.repair? ? '报修申请' : '投诉建议'}", module_id: @repair_complain.repair? ? 1 : 2 }, SystemMessageModule.where(module_id: @repair_complain.repair? ? 1 : 2).first)
  ...
  def telephones
    @share_image = @wx_plot.activity_wx_plot_telephone.try(:qiniu_pic_url)
    @categories = @wx_plot.telephone_categories
    @phones = @wx_plot.wx_plot_telephones
  end

  def lives
    @share_image = @wx_plot.activity_wx_plot_life.try(:qiniu_pic_url)
  ...
  def owners
    @share_image = @wx_plot.activity_wx_plot_owner.try(:qiniu_pic_url)
  ...
  private
    ...
    def set_wx_plot_repair_complains
      @repair_complains = params[:type] == 'repair' ? @user.repairs.show : @user.complain_advices.show
    end
