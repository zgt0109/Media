class Mobile::WxPlotsController < Mobile::BaseController
  ...
  before_filter :set_supplier_website, only: [:bulletins, :bulletin]

  def bulletins
    @share_image = @wx_plot.cover_pic.present? ? @wx_plot.cover_pic : @supplier.activity_wx_plot_bulletin.try(:qiniu_pic_url)
  ...
  def bulletin
    ...
    @share_image = @bulletin.pic.present? ? @bulletin.pic : @supplier.activity_wx_plot_bulletin.try(:qiniu_pic_url)
    render layout: 'mobile/wx_plot_website'
  ...
  def repair_complains
    @share_image = params[:type] == 'repair' ? @supplier.activity_wx_plot_repair.try(:qiniu_pic_url) : @supplier.activity_wx_plot_complain.try(:qiniu_pic_url)
  end

  def repair_complain
    @share_image = params[:type] == 'repair' ? @supplier.activity_wx_plot_repair.try(:qiniu_pic_url) : @supplier.activity_wx_plot_complain.try(:qiniu_pic_url)
    @message = @repair_complain.messages.new(messageable_id: session[:wx_user_id], messageable_type: 'WxUser')
  end
  ...
  def new_repair_complain
    ...
    @repair_complain = @repair_complains.new({wx_plot_id: @wx_plot.id, wx_user_id: @wx_user.id, status: 0}.merge!(session[params[:type].to_sym].to_h))
  end
  ...
  def create_repair_complain
    ...
    if @repair_complain.save
      @supplier.send_system_message({supplier_id: @supplier.id, content: "#{@repair_complain.created_at.strftime('%H:%M')}收到#{@repair_complain.nickname}用户#{@repair_complain.phone}的#{@repair_complain.wx_plot_category.name}#{@repair_complain.repair? ? '报修申请' : '投诉建议'}", module_id: @repair_complain.repair? ? 1 : 2 }, SystemMessageModule.where(module_id: @repair_complain.repair? ? 1 : 2).first)
  ...
  def telephones
    @share_image = @supplier.activity_wx_plot_telephone.try(:qiniu_pic_url)
    @categories = @wx_plot.telephone_categories
    @phones = @wx_plot.wx_plot_telephones
  end

  def lives
    @share_image = @supplier.activity_wx_plot_life.try(:qiniu_pic_url)
  ...
  def owners
    @share_image = @supplier.activity_wx_plot_owner.try(:qiniu_pic_url)
    @owners = @supplier.wbbs_communities.setted
  end
  ...
  private
    ...
    def set_supplier_website
      @website_setting = @supplier.website.try(:website_setting)
