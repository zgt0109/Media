...
class Mobile::ShopTableOrdersController < Mobile::BaseController
  ...
  def create
    ...
    if @shop_table_order.save
      ...
      #   @shop_table_order.site.send_message(shop_branch.mobile, "订座通知：用户#{@shop_table_order.wx_user.nickname}（手机号：#{@shop_table_order.mobile}）于 #{Time.now.to_s} 预定了#{shop_branch.name}分店的座位", false, operation_id: 3, site_id: @site.site_id, userable_id: @user.id, userable_type: 'User')
      # end
  ...
  def success
    @shop_table_order = ShopTableOrder.find(params[:id])
    begin # 发送消息
      RestClient.post("#{MERCHANT_APP_HOST}/v1/igetuis/igetui_app_message", {role: 'site', role_id: @shop_table_order.site_id, token: @shop_table_order.site.account.try(:token), messageable_id: @shop_table_order.id, messageable_type: 'ShopTableOrder', source: 'shop_table_order', message: '您有一笔新的微餐饮订单, 请尽快处理'})
    rescue => e
      Rails.logger.info "#{e}"
    end
