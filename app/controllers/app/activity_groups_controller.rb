  ...
  class ActivityGroupsController < BaseController
    ...
    def show
      ...
      if @activity.activity_groups.where(user_id: session[:user_id]).count > 0
        redirect_to app_activity_groups_path(id: @activity.id)
      end
    end

    def new
      return redirect_to app_activity_groups_path(id: @activity.id) if @activity.activity_groups.where(user_id: session[:user_id]).count>0
