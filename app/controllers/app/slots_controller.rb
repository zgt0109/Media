class App::SlotsController < App::BaseController
        ...
        # if lottery_draws
          if @activity.activity_property.day_partake_limit == -1 or lottery_draws.where(user_id: session[:user_id]).today.count < @activity.activity_property.day_partake_limit #每人每天参与次数
            if @activity.activity_property.partake_limit == -1 or lottery_draws.where(user_id: session[:user_id]).count < @activity.activity_property.partake_limit #每人参与总次数
              ...
              @activity_element_ids =  @activity.generate_rand_slot_element_ids.split(',').map(&:to_i)
              lottery_draw = @activity.lottery_draws.create(site_id: @activity.site_id, user_id: session[:user_id], status: 0)
              render 'fail.js'
              return
            else
  ...
  def find_activity
    ...
    @activity_consumes = @activity.activity_consumes.where(user_id: session[:user_id]).order('id desc') rescue []
