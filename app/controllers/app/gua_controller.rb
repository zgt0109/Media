module App
  class GuaController < BaseController
    before_filter :block_non_wx_browser, :require_wx_mp_user

    def show
      @activity_notice = ActivityNotice.where(id: params[:anid]).first
      @activity = @wx_mp_user.activities.gua.find_by_id(params[:id]) || @activity_notice.try(:activity)
    ...
    def gua
      ...
      if @activity.setted? && @activity.activity_status == Activity::UNDER_WAY
        ...
        if params[:source].present? && params[:source].eql?('notice')
          ...
          if lottery_draws
            if @activity.activity_property.day_partake_limit == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id]).today.count < @activity.activity_property.day_partake_limit #每人每天参与次数
              if @activity.activity_property.partake_limit == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id]).count < @activity.activity_property.partake_limit #每人参与总次数
                if @activity.activity_property.day_prize_limit == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id]).today.win.count < @activity.activity_property.day_prize_limit #每人每天中奖次数
                  if @activity.activity_property.prize_limit == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id]).win.count < @activity.activity_property.prize_limit #每人总中奖次数
                    prize = @activity.get_prize #获取奖品

                    if prize #如果中奖
                      logger.info "========获取到奖券"
                      if prize.people_day_limit_count == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id], activity_prize_id: prize.id).today.count < prize.people_day_limit_count # 某奖品每人每天次数
                        if prize.people_limit_count == -1 || lottery_draws.where(wx_user_id: session[:wx_user_id], activity_prize_id: prize.id).count < prize.people_limit_count # 某奖品每人总次数
                ...
                lottery_draw = @activity.lottery_draws.create(supplier_id: @activity.supplier_id, wx_mp_user_id: @activity.wx_mp_user_id, wx_user_id: session[:wx_user_id], activity_prize_id: (@is_win ? (prize ? prize.id : nil) : nil), status: (@is_win ? LotteryDraw::WIN : LotteryDraw::UNWIN) ) if session[:wx_user_id]
              else
    ...
    def gua_list
      ...
      @activity_consumes = wx_user ? wx_user.activity_consumes.includes(:activity_prize).where(activity_id: session[:activity_id]).order("created_at DESC") : []
    end
