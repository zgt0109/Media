class Biz::VipGroupsController < Biz::VipController
  ...
  def search_user
    @vip_users = current_site.vip_users.visible.name_like(params[:name]).by_group(params[:group])
    @vip_users = @vip_users.includes(:vip_grade, :vip_group).page(params[:page]).per(PAGE_SIZE)
  end

  def search_grade
    @vip_users = current_site.vip_users.visible.by_group(params[:group]).includes(:vip_grade).by_grade(params[:grade])
    @vip_users = @vip_users.page(params[:page]).per(PAGE_SIZE)
  end

  def search_custom_value
    @vip_users = current_site.vip_users.visible.by_group(params[:group]).includes(:vip_grade)
    @vip_users = @vip_users.by_custom_field(params[:custom_field_id], params[:custom_value])
    @vip_users = @vip_users.page(params[:page]).per(PAGE_SIZE)
  end

  def change_group
    @vip_users = current_site.vip_users.visible.includes(:custom_values).by_group(params[:group])
    @vip_users = @vip_users.includes(:vip_grade, :vip_group).page(params[:page]).per(PAGE_SIZE)
  end

  def add_to_group
    vip_group_id = params[:group] == '-2' ? nil : params[:group]
    vip_users = current_site.vip_users.visible.where(id: params[:ids])
  ...
  def remove_user_group_id
    @vip_user = current_site.vip_users.visible.find params[:id]
  ...
  private
  def find_vip_groups
    @vip_groups = current_site.vip_card.vip_groups
  end
