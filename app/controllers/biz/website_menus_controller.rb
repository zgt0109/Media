class Biz::WebsiteMenusController < ApplicationController
  ...
  def new
    ...
    # @ec_seller_cat_selects = wshop_api_categories(wx_mp_user_open_id: @website_menu.website.try(:site).try(:wx_mp_user).try(:openid)).to_a.slice(0, 2)
  ...
  def edit
    ...
    #   @ec_seller_cat_selects = wshop_api_categories(wx_mp_user_open_id: @website_menu.website.try(:site).try(:wx_mp_user).try(:openid), category_id: @website_menu.menuable_id).to_a.slice(0, 2)
    # else
    #   @ec_seller_cat_selects = wshop_api_categories(wx_mp_user_open_id: @website_menu.website.try(:site).try(:wx_mp_user).try(:openid)).to_a.slice(0, 2)
    # end
    @ec_seller_cat_selects = [[1, []]] unless @ec_seller_cat_selects.present?
  ...
  def select_ec_category
    @ec_seller_cat_selects = []#wshop_api_categories(wx_mp_user_open_id: @website.try(:site).try(:wx_mp_user).try(:openid), category_id: params[:category_id])
