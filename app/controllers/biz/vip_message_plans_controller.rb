class Biz::VipMessagePlansController < Biz::VipController
  ...
  def index
    @search            = current_user.vip_card.vip_message_plans.order('created_at DESC').search(params[:search])
  ...
  def new
    @plan = current_user.vip_card.vip_message_plans.build
    render :form, layout: 'application_pop'
  end

  def create
    @plan = current_user.vip_card.vip_message_plans.new params[:vip_message_plan]
    save_vip_message_plan
  end

  def edit
    @plan = current_user.vip_card.vip_message_plans.find params[:id]
    render :form, layout: 'application_pop'
  end
