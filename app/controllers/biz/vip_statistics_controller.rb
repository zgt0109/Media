class Biz::VipStatisticsController < Biz::VipController
  ...
  def show_chart
    ...
    @today = Date.today
    # @vip_users = current_site.vip_users.visible.normal.send(@date, today).select('date(created_at) as created_date, count(*) as count').group('date(created_at)').order("created_at asc")
    @categories, @data, @start, @count = chart_data_for_vip_card(current_site,@date,@today)
    @chart = chart_base_line_for_vip_card(@categories, @data, VipCard::DATES[@date]) if @categories.present?
    @today_counts = current_site.vip_users.visible.normal_and_freeze.where("date(created_at) = ?",@today).count
    @yesterday_counts = current_site.vip_users.visible.normal_and_freeze.where("date(created_at) = ?",@today-1.day).count
    @total_counts = current_site.vip_users.visible.normal_and_freeze.count
  end
