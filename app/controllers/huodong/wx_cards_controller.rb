class Huodong::WxCardsController < ApplicationController
	...
	def card_reports
		@wx_card = @wx_mp_user.cards.where(id: params[:search][:consumable_id_eq]).first if params[:search]
	...
	def new
		@wx_card = @wx_mp_user.cards.new(color: Wx::Card::COLORS.first)
		render :form
	end

	def show
		@wx_card = @wx_mp_user.cards.find params[:id]
		render :form
	end

	def create
		@wx_card = @wx_mp_user.cards.new(params[:wx_card])
		if @wx_card.save
  ...
  def confirm_use_consume
    consume = current_site.consumes.wx_card.unused.unexpired.where(code: params[:sn_code]).readonly(false).first
  ...
  def find_consume
	...
	private
    def find_activity
    	@activity = current_site.activities.where(activity_type_id: Activity::WX_CARD, status:1, activityable_type: nil).first
