class Huodong::RedPacket::ReleasesController < ApplicationController
  before_filter :require_wx_mp_user, :require_red_packet_setting
  ...
  def consumes
    @total_releases = current_user.red_packet_releases
  ...
  private
    def find_activities
      @activities = current_user.red_packet_activities.order("id DESC").page(params[:page])
      @activity_plucks = @activities.pluck(:name, :id)
    end

    def require_red_packet_setting
      @red_packet = current_user.red_packet_setting
