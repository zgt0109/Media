...
class FlowReportsController < ApplicationController
  ...
  def index
    ...
    @today_piwik = @supplier.piwik_by_date(Date.today)
    @yesterday_piwik = @supplier.piwik_by_date(Date.yesterday)

    # @today_data = HashWithIndifferentAccess.new @supplier.daily_piwik_data(Date.today)
    # @yesterday_data = HashWithIndifferentAccess.new @supplier.daily_piwik_data(Date.yesterday)
    # if ["today", "yesterday"].include?(@region )
    #   @sort = "nb_actions" if @sort == "avg_time_on_site"
    #   @piwiks = @supplier.hour_piwik_data(Date.send(@region))
    ...
    #   end
    # else
      @categories, @data = PiwikSite.get_recent_data(@supplier.id, @sort, @region)
    # end
    ...
    @piwik_sites = PiwikSite.where(:supplier_id => @supplier.id).where(date: @date_list[0]..@date_list[1]).order('date desc').page(params[:page]).per(params[:per] || 10)

    #@entry_pages_labels = @supplier.piwik_entry_pages_labels
    #@entry_pages_labels = [] if @entry_pages_labels.is_a?(Hash) && @entry_pages_labels["result"] == "error"
    respond_to do |format|
      format.html
      format.xls {
        send_data(xls_content_for(PiwikSite.where(:supplier_id => @supplier.id).where(date: @date_list[0]..@date_list[1])),
  ...
  def yesterday
    @date = Date.yesterday
    @daily_data = @supplier.daily_piwik_data(@date)
    @piwiks = @supplier.hour_piwik_data(@date)
    render :action => "daily_data"
  end

  def recent_30
    @piwiks = PiwikSite.where(:supplier_id => @supplier).order("date desc").limit(30)
  end
  ...
  def set_wx_mp_user
    @wx_mp_user = current_user.wx_mp_user#s.find(params[:id])
