class WxMenusController < ApplicationController
  ...
  def new
    ...
    @wx_menu = @wx_mp_user.wx_menus.new(supplier_id: @wx_mp_user.supplier_id, parent_id: params[:parent_id].to_i, event_type: "click", menu_type: "1", content: "默认文本")
  ...
  def create
    @wx_menu = current_user.wx_mp_user.wx_menus.new(params[:wx_menu])
  ...
  def up_menu
    first_wx_menu = current_user.wx_menus.where(parent_id: params[:parent_id], id: params[:id]).first
    second_wx_menu = current_user.wx_menus.where(parent_id: params[:parent_id], sort: (first_wx_menu.sort-1)).first if first_wx_menu
    if first_wx_menu.blank? || second_wx_menu.blank?
      WxMenu.new_sort(current_user.wx_menus.where(parent_id: params[:parent_id]))
    else
  ...
  private
    def find_wx_menu
      @wx_menu = current_user.wx_menus.where(id: params[:id]).first
