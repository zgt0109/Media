...
class Api::WeixinController < Api::BaseController
  ...
  def service
    ...
    CustomLog::Base.logger('weixin_logs', attrs.to_json)
    render text: @echostr
  ...
  def log_and_respond_error(error_msg, log_msg, plain_text: true)
    @is_success = 0
    CustomLog::Base.logger('wxapi', log_msg)
  ...
  def serve_post_wx_api
    ...
    case @msg_type
      ...
      when 'voice'    then voice_request()
        ...
        CustomLog::Base.logger('wxapi', "request msg_type: #{@msg_type} params: #{params}")
  ...
  def image_request
    ...
    CustomLog::Base.logger('wxapi', "image_request error: #{error.message} => #{error.backtrace}")
