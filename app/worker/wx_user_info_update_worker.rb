class WxUserInfoUpdateWorker
  ...
  def self.fetch_and_save_user_info(wx_user, wx_mp_user)
    if wx_mp_user.can_fetch_wx_user_info? && !wx_user.has_info?
      WxUserInfoUpdateWorker.perform_async(wx_mp_user.uid, wx_user.uid)
    end
  end

  def self.fetch_and_save_user_info!(wx_user, wx_mp_user)
    if wx_mp_user.can_fetch_wx_user_info? && !wx_user.has_info?
      WxUserInfoUpdateWorker.new.perform(wx_mp_user.uid, wx_user.uid)
    end
  rescue => e
  ...
  def perform(mp_user_openid, openid)
    ...
    if mp_user.try(:can_fetch_wx_user_info?)
      attrs = Weixin.get_wx_user_info(mp_user, wx_user.uid)
      return unless attrs
