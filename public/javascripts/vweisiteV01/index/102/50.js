...
*/
	...
	** 页面切换的效果控制
	*/
var Msize = $(".mod-slider li").size(), 	//页面的数目
	page_n			= 1,			//初始页面位置
	initP			= null,			//初值控制值
	moveP			= null,			//每次获取到的值
	firstP			= null,			//第一次获取的值
...
*/
	...
	function init_pageH(){
		...
		var page_h = fn_h();
		var m_h = $(".mod-slider li").height();
		page_h >= m_h ? v_h = page_h : v_h = m_h ;

		//设置各种模块页面的高度，扩展到整个屏幕高度
		$(".mod-slider li").height(v_h);
		$(".p-index").height(v_h);
...
*/
	//绑定事件
	$(".mod-slider li").on('mousedown touchstart',page_touchstart);
	$(".mod-slider li").on('mousemove touchmove',page_touchmove);
	$(".mod-slider li").on('mouseup touchend mouseout',page_touchend);
	...
	function page_touchmove(e){
		...
		if(start||startM){
			...
			page_n == 1 ? indexP = true : indexP = true ;	//false 为不是第一页 true为第一页
		}
		...
		if(moveP&&startM){
			...
			if(!p_b){
				...
				//向下移动
					if(indexP){
						...
						$(".mod-slider li").eq(newM-1).addClass("animate").css("top",-v_h)
						move = true ;
					}else{
						move = false;
					}
			...
			if(!DNmove){
				...
				if(move){
					...
					//移动中设置页面的值（top）
					start = false;
					var topV = parseInt($(".mod-slider li").eq(newM-1).css("top"));
					$(".mod-slider li").eq(newM-1).css({'top':topV+moveP-initP});
					initP = moveP;
				}else{
					moveP = null;
				}
	...
	function page_touchend(e){
		...
		if(move){
			...
			if( move_p && Math.abs(moveP) >5 ){
				$(".mod-slider li").eq(newM-1).animate({'top':0},300,"easeOutSine",function(){
					/*
					** 切换成功回调的函数
					*/
					success();
			...
			//返回页面(移动失败)
				...
				position ? $(".mod-slider li").eq(newM-1).animate({'top':-v_h},100,"easeOutSine") : $(".mod-slider li").eq(newM-1).animate({'top':v_h},100,"easeOutSine");
				$(".mod-slider li").eq(newM-1).removeClass("animate");
				start = true;
			}
		}
		/* 初始化值 */
...
*/
	function success(){
		...
		*/
		//设置页面的出现
		$(".mod-slider li").eq(page_n-1).removeClass("show animate");
		$(".mod-slider li").eq(newM-1).removeClass("animate").addClass("show");
		...
		//地图重置
		if($(".mod-slider li").eq(page_n-1).find(".ylMap").length>0&&!mapS){
...
*/
	function initPage(){
		//初始化一个页面
		$(".mod-slider li").eq(page_n-1).addClass("show");
