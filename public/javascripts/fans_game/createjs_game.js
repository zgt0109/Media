...
function setCanvas() {
	...
	} else d = W * c / H, b >= d ? (b = d, stage.x = 0) : stage.x = (b - d) / 2;
	a.width = W;
	a.height = H;
	a.style.width = b + "px";
	a.style.height = c + "px"
...
ImageButton.prototype._addFilter = function(a, b) {
	...
	if (this.filters) {
		for (var c in this.filters)
			if (this.filters[c].tag == b) return;
		this.filters.push(a)
	} else this.filters = [a];
	this.do_cache()
};
...
function setFollowAnim(a) {
	if (!USE_NATIVE_SHARE && IS_REFFER) {
		...
		a ? (g_followAnim.play(), createjs.Tween.get(g_followAnim).to({
			regX: b.width,
			regY: 0,
			visible: !0
		}).to({
...
function zero_fill_hex(a, b) {
	for (var c = a.toString(16); c.length < b;) c = "0" + c;
	return c
}
...
function hex2rgb(a) {
	...
	if (a && b.test(a)) {
		if (4 === a.length) {
			for (var b = "#", d = 1; 4 > d; d += 1) b += a.slice(d, d + 1).concat(a.slice(d, d + 1));
			a = b
		}
		b = [];
