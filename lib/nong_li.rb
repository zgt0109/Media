module NongLi
  ...
  def date_format_hs(date)
    unless date.blank?
      date.to_date.strftime("%H:%M") rescue '--'
    else
      '--'
    end
  end
  ...
  # 2011年04月14日
  def date_format_full_l(date)
    unless date.blank?
      date.strftime('%Y年%m月%d日') rescue '--'
    else
      '--'
    end
  end
  ...
  #sample计算今天
  def gong_li_to_nong_to(current_date)
    #today = Date.today
    ...
    nong_li_time = cyclical(l[0]) + "年 " + @@nStr1[l[1]] + "月" + getChinaDate(l[2])
    return nong_li_time
  end
end