...
task :dbdump => :environment do
  ...
  activity_ids = Activity.where(supplier_id: 79750).pluck(:id).join(',')
  fight_paper_ids = FightPaper.where(supplier_id: 79750).pluck(:id).join(',')

  dump = "mysqldump -P1025 -h10.66.46.29 -uroot -p'winwemedia%^&2253' winwemedia_production"
  db = 'keting_db.sql'
  sql_supplier = "\'supplier_id = 79750\'"
  sql_activity = "\'activity_id in (#{activity_ids})\'"
  sql_fight = "\'fight_paper_id in (#{fight_paper_ids})\'"
  sql_wx_mp_user = "\'wx_mp_user_id = 47445\'"
  sql_agent = "\'id = 10001\'"

  system "#{dump} activities --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} activity_consumes --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} activity_hits --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} activity_questions --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} activity_users --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} groups --where=#{sql_supplier} --no-create-info >> #{db}"

  system "#{dump} help_menus --no-create-info >> #{db}"
  system "#{dump} help_posts --no-create-info >> #{db}"


  material_ids = Material.where(supplier_id: 79750).pluck(:id).join(',')
  sql_material = "\'material_id in (#{material_ids})\'"
  system "#{dump} material_contents --where=#{sql_material} --no-create-info >> #{db}"
  system "#{dump} materials --where=#{sql_supplier} --no-create-info >> #{db}"

  system "#{dump} message_hits --where=#{sql_supplier} --no-create-info >> #{db}"

  system "#{dump} payment_settings --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} payment_types --no-create-info >> #{db}"
  system "#{dump} payments --where=#{sql_supplier} --no-create-info >> #{db}"

  system "#{dump} piwik_sites --where=#{sql_supplier} --no-create-info >> #{db}"

  system "#{dump} point_gifts --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} point_transactions --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} point_types --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} vip_cards --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} vip_user_payments --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} vip_user_signs --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} vip_user_transactions --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} vip_users --where=#{sql_supplier} --no-create-info >> #{db}"


  wbbs_topic_ids = WbbsTopic.where(supplier_id: 79750).pluck(:id).join(', ')
  sql_wbbs_vote = "\"votable_type='WbbsTopic' and votable_id in (#{wbbs_topic_ids})\""

  system "#{dump} wbbs_communities --where=#{sql_supplier} --no-create-info >> #{db}"
  system "#{dump} wbbs_replies --where='wbbs_community_id = 4665' --no-create-info >> #{db}"
  system "#{dump} wbbs_topics --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} websites --where=#{sql_supplier} --no-create-info >> #{db}"
  ...
  system "#{dump} wx_menus --where=#{sql_supplier} --no-create-info >> #{db}"
